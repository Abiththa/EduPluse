...
let RoadmapProgress;
try {
  RoadmapProgress = require("../models/RoadmapProgress");
} catch (error) {
...
const getCareerRoadmapByRoleId = async (req, res, next) => {
  try {
    ...
    const roadmap = await CareerRoadmap.findOne({ careerRoleId: id }).populate(
      "careerRoleId",
      "title description"
    );
    ...
    if (!roadmap) {
      return res.status(404).json({
        success: false,
        message: "Roadmap not found for this career role",
      });
    }
  ...
  try {
    if (!RoadmapProgress) {
      return res.status(501).json({
        success: false,
        message: "Roadmap progress model not set up yet",
      });
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        careerRoleId: progress.careerRoleId,
...
const updateRoadmapProgress = async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(completedStepIndexes)) {
      return res.status(400).json({
        success: false,
        message: "completedStepIndexes must be an array",
      });
    }
    ...
    const progress = await RoadmapProgress.findOneAndUpdate(
      ...
      },
      {
        userId: req.user.id,
        careerRoleId: id,
        completedStepIndexes: sanitizedIndexes,
      },
      {
        new: true,
        upsert: true,
