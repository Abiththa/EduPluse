...
const getCvById = async (req, res, next) => {
  try {
    const cv = await Cv.findOne({
      _id: req.params.id,
      userId: req.user.id,
    }).populate("templateId", "name previewImageUrl isActive htmlTemplate");
...
const createCv = async (req, res, next) => {
  try {
    ...
    const createdCv = await Cv.findById(cv._id).populate(
      "templateId",
      "name previewImageUrl isActive"
    );
...
const updateCv = async (req, res, next) => {
  try {
    ...
    const updatedCv = await Cv.findById(cv._id).populate(
      "templateId",
      "name previewImageUrl isActive"
    );
...
const getCvTemplates = async (req, res, next) => {
  try {
    const templates = await CvTemplate.find({ isActive: true }).sort({
      createdAt: -1,
    });
...
const downloadCvPdf = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${safeName || "cv"}-${String(templateName)
        .toLowerCase()
        .replace(/\s+/g, "-")}.pdf"`
    );
