...
const getResumeScoreHistory = async (req, res, next) => {
  try {
    const history = await ResumeScore.find({ userId: req.user.id })
      .populate("cvId", "status updatedAt")
      .populate("targetRoleId", "title keywords")
      .sort({ createdAt: -1 });
...
const deleteResumeScore = async (req, res, next) => {
  try {
    ...
    if (!deletedScore) {
      return res.status(404).json({
        success: false,
        message: "Resume score history item not found",
      });
    }

    res.status(200).json({
      success: true,
      message: "Resume score history item deleted successfully",
    });
  } catch (error) {
    next(error);
  }
