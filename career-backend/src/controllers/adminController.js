...
const createCareerRole = async (req, res, next) => {
  try {
    ...
    if (!title || !title.trim()) {
      return res.status(400).json({
        success: false,
        message: "Career role title is required",
      });
    }
...
const getRoadmapsAdmin = async (req, res, next) => {
  try {
    const roadmaps = await CareerRoadmap.find()
      .populate("careerRoleId", "title description")
      .sort({ createdAt: -1 });
...
const createRoadmap = async (req, res, next) => {
  try {
    ...
    if (!isValidObjectId(careerRoleId)) {
      return res.status(400).json({
        success: false,
        message: "Invalid careerRoleId",
      });
    }
    ...
    if (existingRoadmap) {
      return res.status(409).json({
        success: false,
        message: "Roadmap already exists for this career role",
      });
    }
    ...
    const populatedRoadmap = await CareerRoadmap.findById(roadmap._id).populate(
      "careerRoleId",
      "title description"
    );
...
const updateRoadmap = async (req, res, next) => {
  try {
    ...
    if (careerRoleId !== undefined) {
      ...
      const duplicateRoadmap = await CareerRoadmap.findOne({
        _id: { $ne: roadmap._id },
        careerRoleId,
      });
      ...
      if (duplicateRoadmap) {
        return res.status(409).json({
          success: false,
          message: "Another roadmap already exists for this career role",
        });
      }
    ...
    if (steps !== undefined) {
      roadmap.steps = Array.isArray(steps) ? steps : [];
    }
...
const getJobRolesAdmin = async (req, res, next) => {
  try {
    const jobs = await JobRole.find()
      .populate("linkedCareerRoleId", "title description")
      .sort({ createdAt: -1 });
...
const createJobRole = async (req, res, next) => {
  try {
    ...
    if (linkedCareerRoleId && !isValidObjectId(linkedCareerRoleId)) {
      return res.status(400).json({
        success: false,
        message: "Invalid linkedCareerRoleId",
      });
    }
    ...
    if (linkedCareerRoleId) {
      ...
      if (!role) {
        return res.status(404).json({
          success: false,
          message: "Linked career role not found",
        });
      }
    }
    ...
    const jobRole = await JobRole.create({
      title: title.trim(),
      linkedCareerRoleId: linkedCareerRoleId || null,
      requiredSkills: Array.isArray(requiredSkills) ? requiredSkills : [],
      keywords: Array.isArray(keywords) ? keywords : [],
    });

    const populatedJobRole = await JobRole.findById(jobRole._id).populate(
      "linkedCareerRoleId",
      "title description"
    );
...
const updateJobRole = async (req, res, next) => {
  try {
    ...
    if (linkedCareerRoleId !== undefined) {
      if (linkedCareerRoleId !== null && linkedCareerRoleId !== "" && !isValidObjectId(linkedCareerRoleId)) {
        return res.status(400).json({
          success: false,
          message: "Invalid linkedCareerRoleId",
        });
    ...
    if (requiredSkills !== undefined) {
      jobRole.requiredSkills = Array.isArray(requiredSkills) ? requiredSkills : [];
    }

    if (keywords !== undefined) {
      jobRole.keywords = Array.isArray(keywords) ? keywords : [];
    }
